...
class mod_colibriv2_renderer extends plugin_renderer_base {
     ...
     * @return string
     */
    public function display_meeting_detail_colibri ($meetingdetail, $cmid, $groupid = 0) 
    {
    
#print "<pre>";
#print_r($meetingdetail);
#print "</pre>";

        $attributes = array('scrolling' => 'no',
                            'style' => 'border: 0px;',
                            'height' => '300px',
                            'width' => '100%',
                            'src' => 'https://colibri-devel.local/js-addons/script-iframe.php?meeting=1559043748'
                            );

        $html = html_writer::tag('iframe', '', $attributes);
    ...
    public function display_meeting_detail ($meetingdetail, $cmid, $groupid = 0) {
        ...
        if ($CFG->colibri_mode) {
          return $this->display_meeting_detail_colibri($meetingdetail, $cmid, $groupid = 0);
        }

        $target = new moodle_url('/mod/colibriv2/view.php');
        ...
        $html .= html_writer::tag('label', get_string('meetingname', 'colibriv2'), $param);
        ...
        if ($meetingdetail->url) {
            ...
            $html .= html_writer::tag('label', get_string('meeturl', 'colibriv2'), $param);
            $html .= html_writer::end_tag('div');
        ...
        if ($meetingdetail->servermeetinginfo) {
            ...
            $html .= html_writer::tag('label', get_string('meetinfo', 'colibriv2'), $param);
            $html .= html_writer::end_tag('div');
...
//            $html .= html_writer::tag('label', $meetingdetail->url, $param);
            $html .= html_writer::link($meetingdetail->servermeetinginfo, get_string('meetinfotxt', 'colibriv2'), $param);
            $html .= html_writer::end_tag('div');
        ...
        $html .= html_writer::start_tag('div', $param);
        $param = array('for' => 'lblmeetingurl');
        $html .= html_writer::tag('label', get_string('meetingstart', 'colibriv2'), $param);
        $html .= html_writer::end_tag('div');
        ...
        $html .= html_writer::tag('label', get_string('meetingend', 'colibriv2'), $param);
        $html .= html_writer::end_tag('div');
        ...
        $html .= html_writer::tag('label', get_string('meetingintro', 'colibriv2'), $param);
        $html .= html_writer::end_tag('div');
        ...
        $html .= html_writer::tag('label', format_module_intro('colibriv2', $meetingdetail, $cmid), $param);
        $html .= html_writer::end_tag('div');
        ...
        $target = new moodle_url('/mod/colibriv2/join.php', $param);

        $param = array('type'=>'button',
                       'value'=>get_string('joinmeeting','colibriv2'),
                       'name'=>'btnname',
        ...
        $param = array('type'=>'submit',
                       'value'=>get_string('selectparticipants','colibriv2'),
                       'name'=>'participants',
                       );
        $html .= html_writer::empty_tag('input', $param);
        $html .= html_writer::end_tag('div');
    ...
    function display_meeting_recording($recordings, $cmid, $groupid, $sourcescoid) {
        ...
        $html .= html_writer::tag('h3', get_string('recordinghdr', 'colibriv2'), $param);
    ...
    function display_no_groups_message() {
        $html = html_writer::tag('p', get_string('usergrouprequired', 'colibriv2'));
        return $html;
    }
}