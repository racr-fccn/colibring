...
$url = new moodle_url('/mod/colibriv2/participants.php', array('contextid' => $contextid));
...
list($assignableroles, $assigncounts, $nameswithcounts) =
    colibriv2_get_assignable_roles($context, ROLENAME_BOTH, true);
...
if ($roleid) {
    ...
    if (optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
        ...
        if (!empty($userstoassign)) {
            ...
            add_to_log($course->id, 'role', 'assign', 'mod/colibriv2/participant.php?contextid='.$context->id.'&roleid='.$roleid, $rolename, '', $USER->id);
    ...
    if (optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
        ...
        if (!empty($userstounassign)) {
            ...
            $rolename = $assignableroles[$roleid];
            add_to_log($course->id, 'role', 'unassign', 'mod/colibriv2/participant.php?contextid='.$context->id.'&roleid='.$roleid, $rolename, '', $USER->id);
            // Counts have changed, so reload.
            list($assignableroles, $assigncounts, $nameswithcounts) = get_assignable_roles($context, ROLENAME_BOTH, true);
        }
    }
